...
#define INA  2
#define OUTA 3
// voeg doxygen tag voor deze defines
...
const char *appEui = "0004A30B001F6B25";
const char *appKey = "72AACE3DE2F5F01A321D97FC7250AE8A";
...
// Cayennel LPP
#define APPLICATION_PORT_CAYENNE  99   ///< LoRaWAN port to which kermispackets shall be sent
#define LPP_PAYLOAD_MAX_SIZE      51   ///< Maximum payload size of a LoRaWAN packet

#define LPP_CH_TEMPERATURE        0    ///< kermisCHannel for Temperature
#define LPP_CH_HUMIDITY           1    ///< kermisCHannel for Humidity sensor
#define LPP_CH_LUMINOSITY         2    ///< kermisCHannel for Luminosity sensor
#define LPP_CH_ROTARYSWITCH       3    ///< kermisCHannel for Rotary switch
#define LPP_CH_ACCELEROMETER      4    ///< kermisCHannel for Accelerometer
#define LPP_CH_BOARDVCCVOLTAGE    5    ///< kermisCHannel for Processor voltage
#define LPP_CH_PRESENCE           6    ///< kermisCHannel for Alarm
#define LPP_CH_SET_INTERVAL       20   ///< kermisCHannel for setting downlink interval
...
void loop(){
  ...
  lpp.addDigital(LPP_CH_ROTARYSWITCH, rotaryPosition, IND);
  lpp.addAccelerometer(LPP_CH_ACCELEROMETER, x, y, z);
  lpp.addAnalog(LPP_CH_BOARDVCCVOLTAGE, vdd, INA);
  lpp.addPresence(LPP_CH_PRESENCE, SAFE);
  lpp.addAnalog(LPP_CH_SET_INTERVAL, (float)currentInterval/1000, OUTA);
...
//ja ga deze warning niet fixen, niet mijn gedeelte dus niet mijn probleem (en ik weet niet of het uberhaupt byte vermindering op zou leveren zou why bother)
{
    ...
    default:
      {
        for (int i = 0; i < size; i++){
          ...
          // ik heb je error gezien hiervan maar ga het niet fixen
        }
        debugSerial.println();
      }
...
static void sleep(uint32_t delay_time_ms){
  ...
  while (delay_time_ms)
  {
    ...
    if(alarm){
      ...
      //lpp.addDigital(LPP_CH_SW_RELEASE, RELEASE, IN);
      //lpp.addDigitalInput(LPP_CH_SW_RELEASE, RELEASE);
      lpp.addDigital(LPP_CH_SW_RELEASE, RELEASE, IND);
